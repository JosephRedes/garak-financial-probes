...
from garak_financial.utils import SecureLLMClient, LLMClientError, mask_url
...
@click.option(
    "--batch",
    is_flag=True,
    help="Batch mode: collect all target responses first, then judge. Much faster for local models.",
)
def main(
    target_url: str,
    target_model: str,
    ...
    Examples:
      ...
      garak-financial-assess --target-url ... --target-model ... --buffs base64,persona,jailbreak_prefix
...
def _run_assessment(
    ...
    if max_prompts and len(all_prompts) > max_prompts:
        console.print(f"[yellow]Limiting to {max_prompts} prompts (from {len(all_prompts)})[/yellow]")
        all_prompts = all_prompts[:max_prompts]
    
    console.print(f"[green]Total prompts to test: {len(all_prompts)}[/green]")
    console.print(f"[dim](Base: {aggregator.result.base_prompts}, Augmented: {len(all_prompts) - aggregator.result.base_prompts})[/dim]")
